...
* ``非模块化资源``：虽然在模块化开发体系内，应该 ``一切皆模块``，但总有不应该成为模块的资源，比如入口页面、[模块化框架](/#!/todo)、页面启动器等。
...
非模块化文件通过 [模块化框架](/#!/todo) 来异步加载需要的模块，比如项目的主页面：
